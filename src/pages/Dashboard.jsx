// Dashboard.jsx — v7.0 — Zero re-fetch on tab switch + session cache
...
// ── session-level cache so switching tabs doesn't re-fetch ──────────────────
const _dashCache = { data: null, uid: null, ts: 0 };
const DASH_TTL   = 3 * 60 * 1000; // 3 min
...
export default function Dashboard({ setPage, setResultData, exams, showToast }) {
  ...
  // Restore saved tab from NavBar notification click
  useEffect(() => {
  ...
  // ── Main data load — runs once per user session (cached) ─────────────────
  useEffect(() => {
    if (!user) return;
    let cancelled = false;

    // Return from cache if same user & within TTL
    if (
      _dashCache.uid === user.uid &&
      Date.now() - _dashCache.ts < DASH_TTL &&
      _dashCache.data
    ) {
      const c = _dashCache.data;
      setResults(c.results);
      setSubscription(c.subscription);
      setNotifications(c.notifications);
      setReferralData(c.referralData);
      setLeaderboardPreview(c.leaderboardPreview);
      setExamStats(c.examStats);
    ...
    const loadAll = async () => {
      try {
        ...
        // Batch: notifications + referral + leaderboard + stats
        const uniqueExamIds = [...new Set(userResults.map(r => r.examId))].slice(0, 10);
        const [notifSnap, referralSnap, lbSnap, statsArr] = await Promise.all([
          ...
          Promise.all(uniqueExamIds.map(examId =>
        ...
        if (!cancelled) {
          ...
          const refData  = referralSnap?.exists?.() ? referralSnap.data() : null;
          const lbData   = lbSnap.docs.map((d, i) => ({ id: d.id, rank: i + 1, ...d.data() }));
          const statsMap = {};
          statsArr.forEach(item => { if (item) statsMap[item.examId] = item.stat; });

          setNotifications(notifs);
          setReferralData(refData);
          setLeaderboardPreview(lbData);
          ...
          _dashCache.data = {
            results: recentResults, subscription: sub,
            notifications: notifs, referralData: refData,
            leaderboardPreview: lbData, examStats: statsMap,
          };
        }
      } catch (err) {
  ...
  }, [user?.uid]); // ← ONLY re-runs on user change, NOT on tab switch

  // Tab event from NavBar
  useEffect(() => {
  ...
  // Mark notification read — deduplicated
  const markRead = useCallback(async (id) => {
    ...
    try {
      ...
      // Update cache
      if (_dashCache.data) {
        _dashCache.data.notifications = _dashCache.data.notifications.map(
  ...
  const { passed, failed, avg, totalTime, passRate, certificatesCount, domainStats } = useMemo(() => {
    ...
    return { passed, failed, avg, totalTime, passRate, certificatesCount: certCount, domainStats: domain };
  }, [results]);
  ...
  return (
    <div style={{ maxWidth: 1300, margin: "0 auto", padding: "48px clamp(20px,4vw,48px) 80px" }}>
      ...
      <div className="dash-profile" style={{ display: "flex", alignItems: "center", gap: 24, marginBottom: 32, flexWrap: "wrap", background: "linear-gradient(135deg,var(--bg2) 0%,var(--accent-soft,rgba(99,102,241,0.06)) 100%)", border: "1.5px solid rgba(99,102,241,0.2)", borderRadius: 24, padding: "32px 40px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ width: 80, height: 80, borderRadius: 20, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, fontWeight: 900, color: "#fff", flexShrink: 0, boxShadow: "0 12px 32px rgba(99,102,241,0.3)", position: "relative", zIndex: 1 }}>{initial}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 10, marginBottom: 28 }}>
        {[
          ...
          { label: "🔔 Notifications", page: null,          color: "#ec4899", desc: `${unreadCount} unread`, action: () => setTab("notifications") },
        ].map((btn, i) => (
          <button key={i}
      ...
      {loading ? (
          ...
          <div className="dash-cards" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(360px,1fr))", gap: 20 }}>
            ...
            <Card>
              ...
              {leaderboardPreview.length === 0 ? (
                <p style={{ color: "var(--text3)", fontSize: 13 }}>Complete exams to earn points!</p>
                ...
                return (
                  <div key={entry.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: i < leaderboardPreview.length - 1 ? "1px solid var(--border)" : "none" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: isMe ? 900 : 700, color: isMe ? "var(--accent)" : "var(--text)" }}>{entry.name || "Anonymous"}{isMe && " (you)"}</div>
            ...
            <Card>
              ...
              {referralData?.code && (
                <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}?ref=${referralData.code}`); showToast({ msg: "🔗 Referral link copied!", type: "success" }); }}
                    style={{ padding: "6px 12px", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 8, color: "var(--accent)", fontWeight: 700, cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>
                    Copy Link
                  </button>
                </div>
              )}
        ...
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
            ...
            {notifications.some(n => !n.isRead) && (
              <button onClick={async () => {
                ...
                if (_dashCache.data) _dashCache.data.notifications = _dashCache.data.notifications.map(n => ({ ...n, isRead: true }));
          ...
          {notifications.length === 0 ? (
            ...
            const isSubPurchase  = n.data?.planId && (n.type === "payment_success" || n.type === "instapay_approved");
            const isExamPurchase = n.data?.examTitle && !n.data?.planId && (n.type === "payment_success" || n.type === "instapay_approved");
            return (
              <div key={n.id} onClick={() => markRead(n.id)}
                style={{ display: "flex", gap: 14, padding: "14px 12px", borderBottom: idx < notifications.length - 1 ? "1px solid var(--border)" : "none", cursor: "pointer", borderRadius: 12, background: n.isRead ? "transparent" : meta.bg, border: n.isRead ? "none" : `1px solid ${meta.color}22`, marginBottom: 6, transition: "all 0.2s" }}
              ...
              >
                <div style={{ width: 44, height: 44, borderRadius: 14, background: meta.bg, border: `1.5px solid ${meta.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>{meta.icon}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                    {isSubPurchase && <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: "rgba(99,102,241,0.12)", color: "var(--accent)", border: "1px solid rgba(99,102,241,0.25)" }}>🔄 Subscription · {n.data.planId === "monthly" ? "Monthly" : "Yearly"} Plan</span>}
                    {isExamPurchase && <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: "rgba(6,182,212,0.12)", color: "#0891b2", border: "1px solid rgba(6,182,212,0.25)" }}>🛒 Exam Purchase</span>}
                    {n.data?.examTitle && !n.data?.planId && <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: "rgba(6,182,212,0.08)", color: "#0891b2", border: "1px solid rgba(6,182,212,0.2)" }}>📋 {n.data.examTitle}</span>}
                    {n.data?.amount && <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: "rgba(16,185,129,0.1)", color: "#10b981", border: "1px solid rgba(16,185,129,0.2)" }}>💰 ${Number(n.data.amount).toFixed(2)}</span>}
                    {n.data?.score != null && <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: n.data.score >= 70 ? "rgba(16,185,129,0.1)" : "rgba(245,158,11,0.1)", color: n.data.score >= 70 ? "#10b981" : "#d97706", border: `1px solid ${n.data.score >= 70 ? "rgba(16,185,129,0.2)" : "rgba(245,158,11,0.2)"}` }}>🎯 Score: {n.data.score}%</span>}
                    {n.coupon && <code style={{ fontSize: 11, fontWeight: 900, color: "#10b981", background: "rgba(16,185,129,0.08)", padding: "2px 8px", borderRadius: 6, letterSpacing: "0.08em", border: "1px solid rgba(16,185,129,0.2)" }}>{n.coupon}</code>}
                    {["payment_success","instapay_approved","refund_pending","refund_approved","refund_rejected","refund_update"].includes(n.type) && (n.data?.transactionId || n.data?.examTitle || n.data?.planId) && (
                      <button onClick={(e) => { e.stopPropagation(); window.dispatchEvent(new CustomEvent("myexams:tab", { detail: "billing" })); setPage("my-exams"); }}
                        style={{ fontSize: 10, fontWeight: 800, padding: "3px 10px", borderRadius: 8, background: "rgba(99,102,241,0.1)", color: "var(--accent)", border: "1px solid rgba(99,102,241,0.3)", cursor: "pointer", fontFamily: "inherit" }}>
                        🧾 View Invoice
                      </button>
                    )}
                  </div>
                </div>
                {!n.isRead && <div style={{ width: 8, height: 8, borderRadius: "50%", background: meta.color, flexShrink: 0, marginTop: 6 }} />}
              </div>
            );
          })}
        </Card>
