// Auth.jsx — Complete & Working (No focus loss)
...
const COUNTRIES = Object.entries(countries.getNames("en")).map(([code, name]) => ({
  code,
  name,
  flag: code.toUpperCase().replace(/./g, char => String.fromCodePoint(127397 + char.charCodeAt()))
}));
...
const FieldWrapper = memo(({ icon, label, error, children, subLabel, htmlFor }) => (
  <div style={{ marginBottom: 20 }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
      ...
      {subLabel && <span style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600 }}>{subLabel}</span>}
    </div>
...
const ParticleDots = memo(() => {
  const particles = [
    ...
    { left: "50%", top: "5%", dur: "14s", size: "8px" },
    { left: "80%", top: "80%", dur: "10s", size: "3px" },
    { left: "35%", top: "65%", dur: "13s", size: "6px" },
    { left: "95%", top: "35%", dur: "8s", size: "4px" },
    { left: "25%", top: "95%", dur: "12s", size: "5px" },
    { left: "65%", top: "25%", dur: "11s", size: "7px" }
  ];
  return (
...
// Main Auth Component
export default function Auth({ setPage, showToast, initialMode }) {
  ...
  // ---------- CRITICAL FIX: Stable updater (no dependencies) ----------
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=Clash+Display:wght@600;700&display=swap');
        :root {
          --font-display: 'Clash Display', 'Plus Jakarta Sans', sans-serif;
          --font-body: 'Plus Jakarta Sans', sans-serif;
        }
        .auth-root * { font-family: var(--font-body); box-sizing: border-box; }
        ...
        .mode-tab.active {
          ...
          color: #fff;
          box-shadow: 0 2px 12px var(--accent-glow, rgba(99,102,241,0.3));
        }
        ...
        .submit-btn {
          ...
          gap: 8px;
          transition: all 0.25s ease;
          box-shadow: 0 4px 20px var(--accent-glow, rgba(99,102,241,0.35));
          margin-top: 8px;
        }
        .submit-btn:hover:not(:disabled) {
          transform: translateY(-2px);
          box-shadow: 0 8px 30px var(--accent-glow, rgba(99,102,241,0.5));
        }
        ...
        input:focus, select:focus {
          ...
          box-shadow: 0 0 0 3px var(--accent-glow, rgba(99,102,241,0.2)) !important;
          outline: none;
        }
        .form-animated {
      ...
      <div className="auth-root" style={{ display: "flex", minHeight: "calc(100vh - 74px)", alignItems: "stretch" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "32px clamp(16px, 5vw, 48px)", position: "relative" }}>
          <div className="form-animated" style={{ width: "100%", maxWidth: 448, position: "relative", zIndex: 1 }}>
            ...
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <img src="https://i.ibb.co/8gdkTHSp/login.png" alt="ExamPro Logo" style={{ width: 80, height: 80, borderRadius: 22, marginBottom: 20 }} />
              ...
              <p style={{ color: "var(--text2)", fontSize: 14 }}>
                {mode === "login" ? "Your goals are waiting — sign in and conquer them." : "Join 100,000+ achievers on the path to certification success."}
              </p>
            </div>

            {/* Tabs */}
            <div className="mode-tabs">
              <button className={`mode-tab ${mode === "login" ? "active" : "inactive"}`} onClick={() => { setMode("login"); setErrors({}); }}>Sign In</button>
              <button className={`mode-tab ${mode === "register" ? "active" : "inactive"}`} onClick={() => { setMode("register"); setErrors({}); }}>Create Account</button>
            </div>
            ...
            <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 20, padding: 28, marginBottom: 12 }}>
              ...
              <FieldWrapper icon={Icons.mail} label="Email Address" error={errors.email} htmlFor="email">
                <input
                  id="email"
                  ref={mode === "login" ? firstInputRef : emailRef}
                  type="email"
                  value={form.email}
                  onChange={upd("email")}
                  onKeyDown={handleKey}
              ...
              <FieldWrapper icon={Icons.lock} label="Password" error={errors.password} subLabel={mode === "login" ? "Forgot password?" : null} htmlFor="password">
                <>
                  <input
                    id="password"
                    ...
                    onKeyDown={handleKey}
                    placeholder="••••••••"
                    style={getInputStyle(!!errors.password, true, true)}
                ...
                <>
                  <FieldWrapper icon={Icons.lock} label="Confirm Password" error={errors.confirmPassword} htmlFor="confirmPassword">
                    <input
                      ...
                      onKeyDown={handleKey}
                      placeholder="••••••••"
                      style={getInputStyle(!!errors.confirmPassword, true, true)}
        ...
        {/* Right side - Benefits */}
        <div className="auth-right-panel" style={{ flex: 1, background: "var(--bg2)", borderLeft: "1.5px solid var(--border)", padding: "40px 36px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ textAlign: "center",  position: "relative", zIndex: 1 }}>
            <div style={{ marginBottom: 24 }}>
              <img src="https://i.ibb.co/DPztDcgx/Chat-GPT-Image-Apr-26-2026-09-45-41-PM.png" alt="ExamPro" style={{ height: 100, objectFit: "cover" }} />
            </div>
            ...
            <p style={{ marginBottom: 28, fontSize: 14 }}>
              ExamPro is engineered around your ambitions — adaptive practice, real-world certifications, and a community of 100K+ driven professionals.
            </p>
            {benefits.map((b, i) => (
              <div key={i} className="benefit-item">
                <div style={{ width: 36, height: 36, borderRadius: 10, background: `${b.color}18`, border: `1.5px solid ${b.color}40`, display: "flex", alignItems: "center", justifyContent: "center", color: b.color }}>
                  {benefitIcons[b.icon]}
                </div>
