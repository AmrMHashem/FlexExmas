...
const Icon = ({ name, size = 18, color = "currentColor", style: s }) => {
  const icons = {
    ...
    spinner: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" style={{ animation: "spin 0.8s linear infinite" }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, fontSize: 11, color: "var(--text3)", display: "flex", gap: 6, alignItems: "flex-start" }}>
        ...
        <span>
          {value
            ? "Cancel anytime from account settings. No charges after cancellation."
            : `Access valid until end of ${isYearly ? "yearly" : "monthly"} period. Renew manually when ready.`}
        </span>
      </div>
    </div>
  );
...
function InstapayForm({ amount, description, userId, planId, examId, onSuccess, onError }) {
  ...
  const copyRow = (label, value, copied, onCopy) => (
    ...
    >
      ...
      >
        {copied ? (
          <>
            <Icon name="checkCircle" size={13} color="currentColor" /> Copied!
          </>
        ) : (
          <>
            <Icon name="copy" size={13} color="currentColor" /> Copy
          </>
        )}
      </button>
    </div>
  ...
  return (
    <div>
      ...
      <div
        style={{
          ...infoBox("rgba(99,102,241,0.06)", "rgba(99,102,241,0.2)", "var(--text2)"),
          display: "flex",
          gap: 10,
        }}
      >
        ...
        <span>
          Instapay payments are reviewed manually. Access will be granted within <strong>1–24 hours</strong> after admin
          verification.
        </span>
      </div>
...
>
  ...
  Pay {Number(amountEGP).toLocaleString()} EGP via Instapay App
        ...
        >
          ...
          <button
            onClick={() => copyText(amountEGP, setCopiedAmt)}
            style={{
      ...
      >
        ...
        <span>
          Verification takes <strong>1–24 hours</strong>. You will be notified by email once approved.
        </span>
      </div>
      ...
      >
        {submitting ? (
          <>
            <Spinner size={16} color="#fff" /> Submitting for Verification...
          </>
        ) : (
          <>
            <Icon name="send" size={16} color="#fff" /> Submit Payment for Verification
          </>
        )}
      </button>
    </div>
...
export default function Checkout({ setPage, showToast, checkoutData }) {
  ...
  useEffect(() => {
    const load = async () => {
      const [settings, sub] = await Promise.all([getPlatformSettings(), user ? getUserSubscription(user.uid) : null]);
  ...
  const applyCoupon = async () => {
    ...
    const result = await validateCoupon(couponCode.trim(), isExamPurchase ? examId : null, isExamPurchase ? null : selectedPlan?.id, user.uid);
    setCouponResult(result);
    if (result.valid) {
      couponAppliedRef.current = true;
  ...
  const handleInstapaySuccess = useCallback(
    (data) => {
    ...
    return (
      ...
      >
        ...
        <p style={{ color: "var(--text2)", fontSize: 15, maxWidth: 360, margin: 0, lineHeight: 1.6 }}>
          Your Instapay payment has been submitted for manual verification. Our team will review it within <strong>1–24 hours</strong> and
    ...
    <>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px" }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: "var(--text3)", fontSize: 14, marginTop: 6 }}>{isExamPurchase ? examTitle : "Choose your plan and preferred payment method"}</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 24, alignItems: "start" }}>
            ...
            {grantError && (
              ...
              >
                ...
                >
                  {grantRetrying ? (
                    <>
                      <Spinner size={14} color="#fff" /> Retrying...
                    </>
                  ) : (
                    "Retry Access Grant"
                  )}
                  ...
                  {Object.values(plans).map((plan) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 22, fontWeight: 900, color: "var(--accent)" }}>
                        ${plan.price}
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text3)" }}>/{plan.interval}</span>
                      </div>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  value={couponCode}
                  onChange={(e) => {
                    if (!couponAppliedState) setCouponCode(e.target.value.toUpperCase());
                  }}
                  ...
                  style={{
                    ...
                    fontFamily: "inherit",
                    outline: "none",
                    opacity: couponAppliedState ? 0.7 : 1,
                    textTransform: "uppercase",
                  }}
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 800,
                    cursor: couponAppliedState || !couponCode.trim() ? "default" : "pointer",
                    fontSize: 13,
                    fontFamily: "inherit",
                    display: "flex",
                    gap: 6,
                ...
                >
                  {couponLoading ? (
                    ...
                    <>
                      <Icon name="checkCircle" size={14} color="var(--green)" /> Applied
                    </>
                  ) : (
                    "Apply"
                  )}
          ...
          <div
            style={{
              ...
              padding: 24,
              position: "sticky",
              top: "var(--sticky-top-offset, 84px)",
            }}
          >
            ...
            {(isExamPurchase
              ? ["Full access to all exam questions", "Detailed answer explanations", "Timed & Practice modes", "Official PDF certificate"]
